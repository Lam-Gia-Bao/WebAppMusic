...
<body>
		...
		<div class="mic-container">
			...
			<div
				...
				<div class="text-center flex-grow-1 mic-title">
					...
					<div class="mic-sub">Tải lên bản ghi âm, cập nhật, tin tức
						hoặc phần giới thiệu cho các bản phát hành mới.</div>
				</div>
	...
	<script>
		// Basic drag & drop visual behavior + file open const drop = document.getElementById('dropzone'); const fileInput = document.getElementById('fileInput'); drop.addEventListener('dragenter', e => { e.preventDefault(); drop.style.borderColor = '#5a5a5a'; }); drop.addEventListener('dragover', e => { e.preventDefault(); }); drop.addEventListener('dragleave', e => { e.preventDefault(); drop.style.borderColor = '' ;}); drop.addEventListener('drop', e => { e.preventDefault(); drop.style.borderColor = ''; const files = e.dataTransfer.files; if(files && files.length) handleFiles(files); }); fileInput.addEventListener('change', e => { const files = e.target.files; if(files && files.length) handleFiles(files); }); function handleFiles(files){ // demo: chỉ log tên file, bạn thay bằng upload ajax nếu cần const list = Array.from(files).map(f => f.name).join(', '); alert('Selected files:\\n' + list); } // close button (demo) document.querySelector('.close-circle').addEventListener('click', () => { // nếu là modal trong thực tế: đóng modal. Ở đây: redirect ra trang khác window.history.back(); }); function toggleMicPanel() { const panel = document.getElementById("micPanel"); const arrow = document.getElementById("micArrow"); if (panel.style.display === "block") { panel.style.display = "none"; arrow.classList.remove("bi-chevron-up"); arrow.classList.add("bi-chevron-down"); } else { panel.style.display = "block"; arrow.classList.remove("bi-chevron-down"); arrow.classList.add("bi-chevron-up"); } }
	</script>
</body>
</html>